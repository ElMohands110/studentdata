    ...
    */
    ...
    if (isset($_SESSION['Username'])) {
        ...
        if ($do == 'Manage') { // Manage Page 
            ...
            $stmt = $con->prepare("SELECT * FROM datafirst");
        ...
        ?>
            ...
            <div class="container">
                        ...
                        <tbody>
                            <?php 
                                ...
                                foreach($rows as $row) {
                                    ...
                                    echo "<td>
                                    <a href='firstyear.php?do=Edit&id=" . $row['ID'] . "' class='btn btn-success'><i class='fa fa-edit'></i> Edit</a>
                                    <a href='firstyear.php?do=Delete&id=" . $row['ID'] . "' class='btn btn-danger confirm'><i class='fa fa-times'></i> Delete</a>";
                                    echo "</td>";
                                    echo "</tr>";
                                }
                ...
                <a href="firstyear.php?do=Add" class="btn btn-primary">New Student</a>
            </div>
...
<?php   } elseif ($do == 'Insert') { // Insert Page
            ...
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                ...
                if (empty($formerror)) {
                    ...
                    $checkID = checkItem("ID", "datafirst", $id);
                    $checkName = checkItem("Name", "datafirst", $name);
                    $checkStudentNumber = checkItem("Student_Number", "datafirst", $studentNumber);
                    $checkParrentNumber = checkItem("Parrent_Number", "datafirst", $parrentnumber);
                    ...
                    if ($checkParrentNumber == 1) {
                        ...
                        $stmt = $con->prepare("INSERT INTO 
                                                datafirst(id, `Name`, Student_Number, Parrent_Number, FirstDegree, SecondDegree, ThirdDegree, FourthDegree, FifthDegree, SithDegree, SeventhDegree, Date)
            ...
            $stmt = $con->prepare("SELECT * FROM datafirst WHERE ID = ? LIMIT 1");
...
<?php 
            ...
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                ...
                if (empty($formerror)) {
                    ...
                    $stmt = $con->prepare("UPDATE datafirst SET `Name` = ?, Student_Number = ?, Parrent_Number = ?, FirstDegree = ?, SecondDegree = ?, ThirdDegree = ?, FourthDegree = ?, FifthDegree = ?, SithDegree = ?, SeventhDegree = ? WHERE ID = ?");
            ...
            // If There's Such ID Show The Form
            if ($check > 0) { 
                $stmt = $con->prepare("DELETE FROM datafirst WHERE ID = :zname");

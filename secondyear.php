    ...
    */
    ...
    if (isset($_SESSION['Username'])) {
        ...
        if ($do == 'Manage') { // Manage Page 
            ...
            $stmt = $con->prepare("SELECT * FROM datasecond");
        ...
        ?>
            ...
            <h1 class='text-center'>Second Year</h1>
            <div class="container">
                        ...
                        <tbody>
                            <?php 
                                ...
                                foreach($rows as $row) {
                                    ...
                                    echo "<td>
                                    <a href='secondyear.php?do=Edit&id=" . $row['ID'] . "' class='btn btn-success'><i class='fa fa-edit'></i> Edit</a>
                                    <a href='secondyear.php?do=Delete&id=" . $row['ID'] . "' class='btn btn-danger confirm'><i class='fa fa-times'></i> Delete</a>";
                                    echo "</td>";
                                    echo "</tr>";
                                }
                ...
                <a href="secondyear.php?do=Add" class="btn btn-primary">New Student</a>
            </div>
...
<?php   } elseif ($do == 'Insert') { // Insert Page
            ...
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                ...
                if (empty($formerror)) {
                    ...
                    $checkID = checkItem("ID", "datasecond", $id);
                    $checkName = checkItem("Name", "datasecond", $name);
                    $checkStudentNumber = checkItem("Student_Number", "datasecond", $studentNumber);
                    $checkParrentNumber = checkItem("Parrent_Number", "datasecond", $parrentnumber);
                    ...
                    if ($checkParrentNumber == 1) {
                        ...
                        $stmt = $con->prepare("INSERT INTO 
                                                datasecond(id, `Name`, Student_Number, Parrent_Number, FirstDegree, SecondDegree, ThirdDegree, FourthDegree, FifthDegree, SithDegree, SeventhDegree, Date)
                                                VALUES(:zid, :zname, :zsnumber, :zpnumber, :zfdegree,  :zsdegree,  :ztdegree,  :zfodegree,  :zfidegree,  :zsidegree,  :zsedegree, now()) ");
                        $stmt->execute(Array(
                            'zid'     => $id,
                            'zname'     => $name,
            ...
            $stmt = $con->prepare("SELECT * FROM datasecond WHERE ID = ? LIMIT 1");
                ...
                <div class="container">
                    <form class="form-horizontal" action="?do=Update" method="POST">
                        ...
                        <div class="form-group">
                            <label>Student Number</label>
                            <div class="col-sm-10 col-md-6">
                                <input type="text" name="studentnumber" class="form-control form-control-lg" placeholder="Student Number" required="required" value="<?php echo $row['Student_Number'] ?>"/>
                            </div>
                        </div>  
                        ...
                        <div class="form-group">
                            <label>Parrent Number</label>
                            <div class="col-sm-10 col-md-6 password">
                                <input type="text" name="parrentnumber" class="form-control form-control-lg"placeholder="Parrent Number" required="required" value="<?php echo $row['Parrent_Number'] ?>"/>
                            </div>
                        </div>  
...
<?php 
            ...
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                ...
                if (empty($formerror)) {
                    ...
                    $stmt = $con->prepare("UPDATE datasecond SET `Name` = ?, Student_Number = ?, Parrent_Number = ?, FirstDegree = ?, SecondDegree = ?, ThirdDegree = ?, FourthDegree = ?, FifthDegree = ?, SithDegree = ?, SeventhDegree = ? WHERE ID = ?");
        ...
        } elseif ($do == 'Delete') { // Delete Student
            ...
            // Select All Data Depend On This ID
            $check = checkItem('id', 'datasecond', $id);

            // If There's Such ID Show The Form
            if ($check > 0) { 
                $stmt = $con->prepare("DELETE FROM datasecond WHERE ID = :zname");
